...
class Team:
    ...
    def final(self):
        return self.name + " remaining units " + str(self.get_units())
    ...
    def target_selection(self, other):
        ...
        for group_id, group in ordered_groups:
            ...
            target_id, target = max(targets.items(),
                                    key=lambda t: t[1].get_damage_taken_eff_pow_ini(group.get_effective_power(),
                                                                                    group.dmgtype))  # get best target
            log_dmg = target.get_damage_taken(group.get_effective_power(),
                                              group.dmgtype)  # damage you'd do, just for logging
...
while immuT.get_units() > 0 and infecT.get_units() > 0:
    # status phase    
    immuT.print_status()
    infecT.print_status()
    print()
...
for boost in range(60, 80):
    ...
    while immuT.get_units() > 0 and infecT.get_units() > 0:
        ...
        units = infecT.get_units() + immuT.get_units()
        if last_units == units:
            real_print("no progress made, exiting this try")
            break
        last_units = units

    real_print(boost, infecT.final(), immuT.final())
