...
a = """[1518-05-08 00:12] falls asleep
...
sleepminutes = defaultdict(list)
for line in b:
    time, event = re.findall("\[(.+)\] (.+)", line)[0]
    time = parse(time)
    # print(time, event)
    if "begins" in event:
        activeguard = int(re.findall("#(\d+)", event)[0])
        # print(activeguard)
        ...
        for i in range(int(diff)):
            minute = (startsleep.minute + i) % 60
            sleepminutes[activeguard].append(minute)
max_id, max_time = 0, 0
for id, time in sleeptimes.items():
    if time > max_time:
        max_id, max_time = id, time
print("guard with max id", max_id, "with time", max_time)
# print(sleeptimes)
minute = Counter(sleepminutes[max_id]).most_common()[0][0]
# print(sleepminutes)
print(minute)
print(minute * max_id)
...
for id, _ in sleepminutes.items():
    times, minute = Counter(sleepminutes[id]).most_common()[0]
    if times > max_times:
        max_id, max_times, max_minute = id, times, minute
print(id * minute)
