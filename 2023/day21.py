...
def test_21(data: np.ndarray, level):
    ...
    if level:  # to run the simulation with convolution, we need to expand the field 10x10
        ...
        steps = int(repeat * 4.5)  # the highest history value we need is at rep*4.5
    ...
    return solve_analytical(hist, repeat, target=26501365) if level else np.sum(data)

def solve_analytical(hist: List[int], repeat: int, target: int):
    ...
    """
    pos_relevant = range(target % repeat, len(hist), repeat * 2)  # 65, 327, 589 for field size of 131
