...
def test_04(data: List[str], level):
    ...
    c = Counter()
    for line in data:
        line = line.replace("  ", " ").replace("Card ", "")
        left, right = line.split(" | ")
        num, left = left.split(": ")
        left = set([int(l) for l in left.split(" ")])
        right = set([int(r) for r in right.split(" ")])
    ...
    if level:
        max_id = max(c.keys())
        ticket_num = Counter(c.keys())
        for src in range(max_id):
            ticket_num += {to_add: ticket_num[src]
                           for to_add in range(src + 1, src + 1 + c[src])
                           if to_add <= max_id}
