...
class Image:
    def __init__(self, img_as_txt: str):
        ...
        self.img = np.equal(np.array([list(i) for i in img]), '#').astype(int)
    ...
    def visit(self, pos=None, must_match=None):
        if self.pos_neigh:
            ...
            for v in variations(self.img):
                self.img = v
                if np.all(self.edge(pos) == must_match):
                    break
...
#    ##    ##    ###
 #  #  #  #  #  #   """
    mask = np.equal(np.array([list(line) for line in monster.split("\n")]), "#").astype(int)
    ...
    for fieldv in variations(field):  # for all field-variations and mask shifts
        ...
        for i, j in itertools.product(range(fx - mx), range(fx - my)):
            fcnt += np.all(fieldv[i:i + mx, j:j + my] & mask == mask)
        if fcnt:
